...
import static oracle.kubernetes.operator.DomainConfigMapKeys.CONFIGURATION_OVERRIDES;
...
public class ConfigMapHelper {
  ...
  /**
   * Creates a stop to test for a change to the configuration overrides. If they have changed,
   * caches them for next time, and adds OVERRIDES_MODIFIED to the packet.
   * @param next the next step to run
   */
  ...
  private static class OverrideDetectionStep extends Step {

    private final Domain domain;

    public OverrideDetectionStep(Domain domain) {
      this.domain = domain;
    }

    @Override
    public NextAction apply(Packet packet) {
      final String overridesState = createOverridesState(domain);
      if (overridesState.equals(packet.get(CONFIGURATION_OVERRIDES))) {
        return doNext(packet);
      } else {
    ...
    Map<String,String> createNewValues(String overridesState) {
      return Map.of(CONFIGURATION_OVERRIDES, overridesState);
    }
  }
}
